...
if ! "$python_command" -c 'import yaml; assert tuple(int(part) for part in yaml.__version__.split(".")[:1]) >= (6,)' 2>/dev/null; then
...
if [[ -n "${RAK_INPUT_POLICY:-}" ]]; then
  args+=(--policy "$RAK_INPUT_POLICY")
fi

set +e
PYTHONPATH="$RAK_ACTION_ROOT/src${PYTHONPATH:+:$PYTHONPATH}" \
  "$python_command" -m research_audit_kit.cli "${args[@]}" >"$stdout_file"
audit_code=$?
set -e
...
if [[ -f "$result_file" ]]; then
  ...
  if [[ -n "${GITHUB_OUTPUT:-}" ]]; then
    audit_status=$("$python_command" -c 'import json,sys; print(json.load(open(sys.argv[1], encoding="utf-8"))["status"])' "$result_file")
